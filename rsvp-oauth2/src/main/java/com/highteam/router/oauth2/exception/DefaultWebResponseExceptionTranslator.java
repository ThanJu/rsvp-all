...
public class DefaultWebResponseExceptionTranslator implements WebResponseExceptionTranslator {
	...
	private ResponseEntity<OAuth2Exception> handleBusinessException(BusinessException e) throws IOException {
		...
		OAuth2Exception ae = new OAuth2Exception(e.getMessage(), e);
		
		ResponseEntity<OAuth2Exception> response = new ResponseEntity<OAuth2Exception>(ae, headers, HttpStatus.OK);
